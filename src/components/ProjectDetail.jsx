...
const TechBadge = ({ tech }) => {
  ...
  return (
    <div className="group relative overflow-hidden px-3 py-2 md:px-4 md:py-2.5
                    bg-gradient-to-r from-blue-600/10 to-cyan-600/10
                    rounded-xl border border-blue-500/10
                    hover:border-blue-400/40 transition-all duration-300 cursor-default">
      <div className="absolute inset-0 bg-gradient-to-r from-blue-500/0 to-cyan-500/0
                      group-hover:from-blue-500/10 group-hover:to-cyan-500/10 transition-all duration-500" />
      <div className="relative flex items-center gap-1.5 md:gap-2">
        <Icon className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-300 group-hover:text-cyan-200 transition-colors" />
        <span className="text-xs md:text-sm font-medium text-blue-200/90 group-hover:text-cyan-100 transition-colors">
          {tech}
        </span>
      </div>
    </div>
...
const FeatureItem = ({ feature }) => (
  <li className="group flex items-start space-x-3 p-2.5 md:p-3.5 rounded-xl
                 hover:bg-white/5 transition-all duration-300
                 border border-transparent hover:border-white/10">
    <div className="relative mt-2">
      <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/20 to-cyan-500/20
                      rounded-full blur group-hover:opacity-100 opacity-0 transition-opacity duration-300" />
...
const ProjectStats = ({ project }) => {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:gap-4 p-3 md:p-4 bg-[#0a0f1e] rounded-xl overflow-hidden relative">
      ...
      <div className="relative z-10 flex items-center space-x-2 md:space-x-3
                      bg-white/5 p-2 md:p-3 rounded-lg
...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const selected = PROJECTS.find((p) => Number(p.id) === projectId);
    if (selected) {
      const enhanced = {
        ...selected,
        Features: selected.Features || [],
        TechStack: selected.TechStack || [],
        Github: selected.Github || 'https://github.com/',
      };
      setProject(enhanced);
    }
  }, [id]);
    ...
    <div className="min-h-screen bg-[#030014] px-[2%] sm:px-0 relative overflow-hidden">
      ...
      <div className="fixed inset-0">
        <div className="absolute -inset-[10px] opacity-20">
          <div className="absolute top-0 -left-4 w-72 md:w-96 h-72 md:h-96
                          bg-blue-500 rounded-full mix-blend-multiply
                          filter blur-3xl opacity-70 animate-blob" />
          <div className="absolute top-0 -right-4 w-72 md:w-96 h-72 md:h-96
                          bg-cyan-500 rounded-full mix-blend-multiply
                          filter blur-3xl opacity-70 animate-blob animation-delay-2000" />
          <div className="absolute -bottom-8 left-20 w-72 md:w-96 h-72 md:h-96
                          bg-sky-500 rounded-full mix-blend-multiply
      ...
      <div className="relative">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
          ...
          <div className="flex items-center space-x-2 md:space-x-4 mb-8 md:mb-12 animate-fadeIn">
            <button
              onClick={() => navigate(-1)}
              className="group inline-flex items-center space-x-1.5 md:space-x-2
            ...
            <div className="space-y-6 md:space-y-10 animate-slideInLeft">
              <div className="space-y-4 md:space-y-6">
                <h1 className="text-3xl md:text-6xl font-bold
                               bg-gradient-to-r from-blue-200 via-cyan-200 to-sky-200
                ...
                {project.Link && (
                  <a
                    ...
                    className="group relative inline-flex items-center space-x-1.5 md:space-x-2
                               ...
                               bg-gradient-to-r from-blue-600/10 to-cyan-600/10
                               hover:from-blue-600/20 hover:to-cyan-600/20
                               text-blue-300 rounded-xl transition-all duration-300
                               border border-blue-500/20 hover:border-blue-400/40
                               backdrop-blur-xl overflow-hidden text-sm md:text-base"
                  >
                ...
                {project.Github && (
                  <a
                    ...
                    className="group relative inline-flex items-center space-x-1.5 md:space-x-2
                               ...
                               text-cyan-300 rounded-xl transition-all duration-300
            ...
            <div className="space-y-6 md:space-y-10 animate-slideInRight">
              ...
              <div className="bg-white/[0.02] backdrop-blur-xl rounded-2xl p-8 border border-white/10 space-y-6 hover:border-white/20 transition-colors duration-300 group">
                <h3 className="text-xl font-semibold text-white/90 flex items-center gap-3">
                  <Star className="w-5 h-5 text-blue-300 group-hover:rotate-[20deg] transition-transform duration-300" />
                  Key Features
                </h3>
